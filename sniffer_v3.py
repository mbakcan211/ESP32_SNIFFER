...
# --- REPLACE THE ENTIRE GraphWindow CLASS WITH THIS ---
class GraphWindow(QMainWindow):
    def __init__(self, mac_address, history_data):
        ...
        # 1. The Graph Area (Left Side)
        self.figure = Figure(facecolor='#121212') 
        self.canvas = FigureCanvas(self.figure)
        layout.addWidget(self.canvas, stretch=3) # Takes up 75% of window
        
        self.ax = self.figure.add_subplot(111)
        self.ax.set_facecolor('#1e1e1e')
        
        # 2. The Stats Dashboard (Right Side)
        stats_panel = QVBoxLayout()
        stats_panel.setContentsMargins(10, 50, 10, 50)
        
        self.lbl_current = self.create_stat_label("CURRENT", "- dBm")
        self.lbl_avg = self.create_stat_label("AVERAGE", "- dBm")
        self.lbl_max = self.create_stat_label("MAX PEAK", "- dBm")
        self.lbl_min = self.create_stat_label("MIN DIP", "- dBm")
        self.lbl_quality = self.create_stat_label("QUALITY", "ANALYZING")

        stats_panel.addWidget(self.lbl_current)
        stats_panel.addWidget(self.lbl_avg)
        stats_panel.addWidget(self.lbl_max)
        stats_panel.addWidget(self.lbl_min)
        stats_panel.addWidget(self.lbl_quality)
        stats_panel.addStretch()
        
        layout.addLayout(stats_panel, stretch=1)

        # Timer
        self.timer = QTimer()
        self.timer.timeout.connect(self.update_plot)
        self.timer.start(500) # Faster update (0.5s)
    ...
    def create_stat_label(self, title, value):
        ...
        val_lbl.setStyleSheet("color: #00FF00; font-size: 24px; font-family: Monospace; font-weight: bold;")
    ...
    def update_plot(self):
        ...
        min_rssi = np.min(y_data)
        
        # Update text
        self.lbl_current.findChild(QLabel, "", Qt.FindChildOption.FindDirectChildrenOnly).nextInFocusChain().setText(f"{curr_rssi} dBm")
        self.lbl_avg.findChild(QLabel, "", Qt.FindChildOption.FindDirectChildrenOnly).nextInFocusChain().setText(f"{avg_rssi:.1f} dBm")
        self.lbl_max.findChild(QLabel, "", Qt.FindChildOption.FindDirectChildrenOnly).nextInFocusChain().setText(f"{max_rssi} dBm")
        self.lbl_min.findChild(QLabel, "", Qt.FindChildOption.FindDirectChildrenOnly).nextInFocusChain().setText(f"{min_rssi} dBm")
        
        # Determine Quality
        q_lbl = self.lbl_quality.findChild(QLabel, "", Qt.FindChildOption.FindDirectChildrenOnly).nextInFocusChain()
        if curr_rssi > -50:
            q_lbl.setText("EXCELLENT")
            q_lbl.setStyleSheet("color: #00FF00; font-size: 24px; font-weight: bold;")
        elif curr_rssi > -70:
            q_lbl.setText("GOOD")
            q_lbl.setStyleSheet("color: #FFFF00; font-size: 24px; font-weight: bold;")
        else:
            q_lbl.setText("POOR")
            q_lbl.setStyleSheet("color: #FF0000; font-size: 24px; font-weight: bold;")

        # --- Draw Graph ---
        self.ax.clear()
        
        # 1. Background Zones (The "Context")
        self.ax.axhspan(-50, 0, facecolor='#004400', alpha=0.2)   # Excellent Zone
        self.ax.axhspan(-70, -50, facecolor='#444400', alpha=0.2) # Good Zone
        self.ax.axhspan(-100, -70, facecolor='#440000', alpha=0.2) # Poor Zone
        
        # 2. The Line & Fill
        self.ax.plot(x_data, y_data, color='#00FFFF', linewidth=2, marker='o', markersize=4, markeredgecolor='white')
        self.ax.fill_between(x_data, y_data, -100, color='#00FFFF', alpha=0.1)
